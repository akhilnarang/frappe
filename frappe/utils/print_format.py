...
def download_multi_pdf(
	doctype, name, format=None, no_letterhead=False, letterhead=None, options=None, print_async=False
):
	"""
	Calls _download_multi_pdf with the given parameters and returns the URL of the generated PDF

	If `print_async` is True, a task ID will be returned that the client can subscribe to for
	progress updates and the final PDF URL.
	"""
	if print_async:
	...
	if not isinstance(doctype, dict):
		...
		for idx, ss in enumerate(result):
			try:
				...
				if frappe.job:
					frappe.publish_realtime(task_id=task_id)

			# Publish progress if background job
			if frappe.job:
				frappe.publish_progress(idx / total_docs * 100, title="Creating PDF")
